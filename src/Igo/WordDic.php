...
class WordDic
{
    ...
    public function __construct($dataDir)
    {
        ...
        $this->data = FileMappedInputStream::getStringStatic($dataDir.'/word.dat');
        $this->indices = FileMappedInputStream::getIntArrayStatic($dataDir.'/word.ary.idx');
    ...
    public function wordData($wordId)
    {
        return substr(
            $this->data,
            ($this->dataOffsets->get($wordId)) << 1,
            ($this->dataOffsets->get($wordId + 1) - $this->dataOffsets->get($wordId)) << 1
        );
    }
}
